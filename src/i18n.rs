...
fn read_file(lang: &realm_lang::Language, res: &'static str) -> Option<String> {
    ...
    let i = issue(lang, res, None);

    let mut s = String::new();
    let mut f = match std::fs::File::open(format!("i18n/{}/terms.ftl", lang.to_2_letter_code())) {
        Ok(v) => v,
        Err(e) => {
            println!("not found {}: {:?}", i, e);
            return None;
        }
    };
    f.read_to_string(&mut s)
        .unwrap_or_else(|_| panic!("{}", i.as_str()));
    let mut f = std::fs::File::open(format!("i18n/{}/{}.ftl", lang.to_2_letter_code(), res))
        .unwrap_or_else(|_| panic!("{}", i));
    f.read_to_string(&mut s).unwrap_or_else(|_| panic!("{}", i));
    Some(s)
}

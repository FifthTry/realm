module Widget.Layout.LeftSidebar exposing (Config, Model, Msg(..), config, init, subscriptions, update, view, widget)
...
config =
    JD.list DE.widgetSpec
        |> JD.andThen
            (\l ->
                case l of
                    [ h, s1, s2, s3, s4, s5, s6, s7, s8, m1, m2, m3, m4, m5, m6, m7, m8, f, _, _, _ ] ->
            ...
            [ L.row_ C.mainPage
                (HE.viewList m.config.sidebar m.sidebar SidebarMsg
                    ++ [ L.column_ C.page
                            ...
                            , L.column_ C.pageWrapper
                                [ E.scrollView
                                    [ R.style C.componentWrapper ]

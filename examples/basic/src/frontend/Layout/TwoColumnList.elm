module Widget.Layout.TwoColumnList exposing (..)
...
import Realm.Style exposing (backgroundColor)
import Style as C
                ...
                case l of
                    [ h, l, r, b, f ] ->
                        case
                            ...
                            , JD.decodeValue (JD.list DE.widgetSpec) l
                            , JD.decodeValue (JD.list DE.widgetSpec) r
                            , JD.decodeValue (JD.list DE.widgetSpec) b
...
view m =
    let
        bg =
            [ backgroundColor C.bgLightestBlue ]
    in
    L.column_ C.wrapper
                ...
                [ L.column_ C.page
                    [ L.column_ C.pageWrapper2
                        [ E.scrollView
                            ...
                            [ L.row_ []
                                [ L.column_ (C.pageWrapper2Left ++ bg)
                                    (HE.viewList m.config.left m.left LeftMsg)
                                , L.column_ (C.pageWrapper2Right ++ bg)
